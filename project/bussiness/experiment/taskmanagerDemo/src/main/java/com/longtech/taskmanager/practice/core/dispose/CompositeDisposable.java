...
public final class CompositeDisposable implements Disposable, DisposableContainer {
    ...
    /**
     * Dispose the contents of the {@link HashSet} by suppressing non-fatal
     * {@link Throwable}s till the end.
     * @param set the {@code HashSet} to dispose elements of
     */
    void dispose(HashSet<Disposable> set) {
        ...
        if (errors != null) {
            if (errors.size() == 1) {
                // Todo:throw new Exception(errors.get(0));
            }
            // Todo:throw new Exception(errors);
        }
    }
}
